...
//=================枝プロンテラNPC改造版(個数制限あり)====================================
//========================================================================================
prontera,166,125,4	script	モンスター召還屋	738,{
	if(Zeny < 50000) {
		mes "[モンスター召還屋]";
		mes "お金が足りません。";
		close;
...
//=====================カウント部===============================
prontera,0,0,0	script	prtmobdie	-1,{
	set $@prtmob,$@prtmob-1;
...
prontera,168,125,4	script	モンスターリセット屋	81,{
	mes "[モンスターリセット屋]";
	mes "モンスターをリセットします。";
	next;
	if(select("はい","いいえ") == 1) {
		killmonster "prontera","prtmobdie";
		mes "[モンスターリセット屋]";
		mes "Mobdelete完了";
		next;
		set $@prtmob,0;
		mapannounce "prontera","[南十字路情報] " + strcharinfo(0) + "によりMobリセットされました。",20;
		mes "[モンスターリセット屋]";
		mes "FlagReset完了";
	}
	donpcevent "モンスター召還屋::OnCount";
