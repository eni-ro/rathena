...
ma_scene01,174,179,4	script	タホー	541,{
	...
	if(checkquest2(12278)) {
		if(!(checkquest2(12278) & 0x2)) {
			mes "[タホー]";
			mes "バコナワ湖に下りるための";
			mes "命綱を使い切ってしまいました。";
			mes "用意に時間がかかるので、";
...
// バコナワの棲み処
//------------------------------------------------------------
1@ma_b,0,0,0	script	BakonawaControl	-1,{
OnStart:
	if(.flag > 0)
		end;
	set .flag,1;
...
1@ma_b,63,52,4	script	タホー#nf	541,{
	mes "[タホー]";
	mes "いよいよバコナワとの戦闘です。";
...
1@ma_b,36,111,4	script	#バコナワn1	844,{
OnStart:
	announce "タホー：バコナワが現れました！　10分以内に倒してください！",0x9,0x00ffff,0x190,15;
	monster getmdmapname("1@ma_b"),78,83,"バコナワ",2320,1,getmdnpcname("#バコナワn1")+"::OnKilled";
	initnpctimer;
	end;
OnKilled:
...
OnTimer600000:
	...
	killmonster getmdmapname("1@ma_b"),getmdnpcname("#バコナワn1")+"::OnKilled";
	stopnpctimer;
	hideonnpc getmdnpcname("#バコナワn1");
	end;
OnAnnounce:
...
1@ma_b,78,81,0	script	#手下召喚n1	139,4,5,{
OnStart:
...
OnTimer50000:
	killmonster getmdmapname("1@ma_b"),getmdnpcname("#手下召喚n1")+"::OnKilled";
	stopnpctimer;
	end;
OnKilled:
	end;
...
1@ma_b,1,5,4	script	#バコナワ討伐n1	844,{
	end;
OnStart:
	initnpctimer;
	end;
...
1@ma_b,36,111,4	script	#バコナワn2	844,{
	end;
OnStart:
...
OnKilled:
	set .@count,getmapmobs(getmdmapname("1@ma_b"),getmdnpcname("#バコナワn2")+"::OnKilled");
	if(.@count < 1) {
...
OnTimer300000:
	announce "バコナワが湖の中に逃げて行きました。",0x9,0xffff00,0x190,20;
	donpcevent getmdnpcname("タホー#失敗")+"::OnStart";
	killmonster getmdmapname("1@ma_b"),getmdnpcname("#バコナワn2")+"::OnKilled";
	stopnpctimer;
	end;
OnAnnounce:
	announce "制限時間"+getarg(0),0x9,0xff4400,0x190,getarg(1);
...
1@ma_b,36,111,4	script	#バコナワn2-1	844,{
	end;
OnInstanceInit:
...
OnStart:
	...
	monster getmdmapname("1@ma_b"),78,93,"バコナワ",2321,1,getmdnpcname("#バコナワn2-1")+"::OnKilled";
	end;
OnEnd:
	enablenpc getmdnpcname("#バコナワn2-1");
	killmonster getmdmapname("1@ma_b"),getmdnpcname("#バコナワn2-1")+"::OnKilled";
	initnpctimer;
	end;
OnTimer1000:
...
1@ma_b,36,111,4	script	#バコナワn3	844,{
	end;
OnStart:
	initnpctimer;
	monster getmdmapname("1@ma_b"),78,83,"Enraged Bakonawa",2322,1,getmdnpcname("#バコナワn3")+"::OnKilled";
...
OnTimer600000:
	announce "バコナワが湖の中に逃げて行きました。",0x9,0xffff00,0x190,20;
	donpcevent getmdnpcname("タホー#失敗")+"::OnStart";
	killmonster getmdmapname("1@ma_b"),getmdnpcname("#バコナワn3")+"::OnKilled";
	stopnpctimer;
	end;
OnAnnounce:
	announce "制限時間"+getarg(0),0x9,0xff4400,0x190,getarg(1);
...
1@ma_b,36,111,4	script	#バコナワn3-1	844,{
	end;
OnStart:
	initnpctimer;
	end;
OnEnd:
	killmonster getmdmapname("1@ma_b"),getmdnpcname("#バコナワn3-1")+"::OnKilled";
	stopnpctimer;
	end;
...
1@ma_b,66,64,0	script	タホー#完了	541,{
	mes "[タホー]";
	mes "すごい戦いでしたね！";
	mes "手下が残っていたら危険なので、";
	mes "上に戻りましょう。";
...
OnTimer1000:
	monster getmdmapname("1@ma_b"),78,74,"バコナワの宝箱",2335,1;
	end;
OnTimer10000:
...
1@ma_b,61,52,4	script	タホー#失敗	541,{
	mes "[タホー]";
	mes "取り逃がしてしまいましたね。";
	mes "湖の底に潜ってしまうと、";
	mes "こちらから手出しできません。";
...
//1@ma_b,62,51,0	warp	#バコナワ入口テレポ	2,2,ma_scene01,175,176
