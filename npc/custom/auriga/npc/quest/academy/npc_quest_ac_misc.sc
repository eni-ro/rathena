...
ac_cl_hall,155,35,4	script	消耗品販売員#ac	93,{/* 52611 */
	callshop "ac_shop1";
	end;
OnInit:
...
ac_cl_hall,152,29,4	script	道具販売員#ac	82,{/* 52612 */
	callshop "ac_shop2";
	end;
OnInit:
...
ac_cl_hall,178,43,4	script	ゼン#ac	752,{/* 52615 */
	callshop "ac_shop3";
	end;
OnInit:
...
ac_cl_hall,182,39,3	script	エーイ#ac	815,{/* 52616 */
	callshop "ac_shop4";
	end;
OnInit:
...
ac_cl_hall,183,44,4	script	コウ#ac	732,{/* 52617 */
	callshop "ac_shop5";
	end;
OnInit:
...
ac_cl_hall,181,35,4	script	モーリ#ac	52,{/* 52618 */
	callshop "ac_shop6";
	end;
OnInit:
...
ac_cl_hall,179,31,4	script	シャーレ#ac	731,{/* 52619 */
	callshop "ac_shop7";
	end;
OnInit:
...
function	script	AC_WarpTown	{
	switch(getarg(0)) {
	case 0: warp "prontera",150,190; break;
	case 1: warp "izlude",126,113; break;
	case 2: warp "morocc",147,93; break;
	case 3: warp "alberta",103,129; break;
	case 4: warp "geffen",117,103; break;
	case 5: warp "payon",155,203; break;
	case 6: warp "einbroch",217,188; break;
	case 7: warp "amatsu",148,134; break;
	case 8: warp "aldebaran",138,110; break;
	}
	return;
}
...
ac_cl_hall,158,34,4	script	銃弾販売員#ac	85,{
	mes "[銃弾販売員]";
	mes "やぁ、こんにちは。";
	mes "ここでは、銃用の弾丸を";
	mes "販売しているよ。";
...
//カプラサービスポスト
ac_cl_area,41,145,4	script	カプラサービス	888,{
	mes "‐(株)カプラサービスは、";
	mes "いつも皆様のそばに‐";
	mes " ";
	mes "‐どのサービスを利用する？‐";
...
//ミッド
ac_cl_area,45,146,4	script	ミッド#ac_area	429,{
	set .@novice, callfunc("AC_GetNovice");
	cutin "jpn_mid01",2;
	if(AC_PASSPORT==2){
		mes "[ミッド]";
...
L_GIVEBOOK:
	...
	if(checkweight(11055,1)) {
		switch(Job){
			...
			case Job_Mage:    getitem 11037,1; break; //マジシャンの書
...
//モッド
ac_cl_area,61,166,4	script	モッド#AC	704,{
	set .@novice, callfunc("AC_GetNovice");
	if (!.@novice) {
		mes "[モッド]";
		mes "こんにちは。";
...
//タール
ac_cl_area,57,173,2	script	タール#AC	931,{
	set .@novice, callfunc("AC_GetNovice");
	if (.@novice) {
		if(AC_RANK==0){
			mes "[タール]";
...
ac_cl_area,57,168,4	script	ヘイム	798,{
	mes "[ヘイム]";
	mes "やぁ、こんにちは。";
	mes "僕はヘイムだよ。";
	next;
...
L_CANCEL:
	...
	close;
OnInit:
	setarray .gatname$,"prontera","izlude","morocc","alberta","geffen","payon","aldebaran","yuno","umbala","comodo","hugel","rachel","lighthalzen","veins","gonryun","louyang","ayothaya","amatsu","einbroch",  "einbech";
...
ac_cl_area,63,81,0	script	3階WP	45,1,1,{
	mes "[3階]";
	mes "3階は一般学生の立ち入りを";
	mes "禁止しています。";
	close;
...
//資料室管理人
ac_cl_area,41,38,4	script	資料室管理人	75,{
	mes "[資料室管理人]";
	mes "ここは資料室になります。";
	mes "いろんな資料をここに";
	mes "集めてるんですよ。";
...
//ヨンお爺さん
ac_cl_area,39,23,4	script	ヨン#ac_cl_area	766,{
	set .@novice, callfunc("AC_GetNovice");
...
L_TENSOU:
	...
	close2;
	switch(.@num){
	case 1: warp "prt_sewb2",70,100; end;
	case 2: warp "prt_fild07",78,287; end;
	case 3: warp "pay_fild02",135,172; end;
	case 4: warp "pay_dun00",108,24; end;
	case 5: warp "pay_fild09",218,289; end;
	case 6: warp "iz_dun00",237,166; end;
	case 7: warp "gef_fild03",32,55; end;
	case 8: warp "pay_dun01",231,74; end;
	case 9: warp "gef_fild11",369,294; end;
	case 11: warp "gef_fild12",125,184; end;
	case 12: warp "mjolnir_05",170,280; end;
	case 13: warp "gef_dun01",247,231; end;
	}
	end;
OnInit:
		...
		if(!getcharid(1)){
			...
			for(set .@i,0; .@i<$@partymembercount; set .@i,.@i+1) {
				if(attachrid(getcharid(3,$@partymembername$[.@i]))){
					set .@dummy, getmapxy(.@pmmap$,.@pmx,.@pmy,BL_PC);
...
prt_fild07,78,290,4		duplicate(アインACFUNC)		アイン#PRTF07	747
prt_sewb2,70,103,4		duplicate(アインACFUNC)		アイン#PRTSB2	747
pay_fild02,135,175,4	duplicate(アインACFUNC)		アイン#PAYF02	747
pay_dun00,108,26,4		duplicate(アインACFUNC)		アイン#PAYD00	747
pay_fild09,218,292,4	duplicate(アインACFUNC)		アイン#PAYF09	747
iz_dun00,237,169,4		duplicate(アインACFUNC)		アイン#IZLD00	747
gef_fild03,32,58,4		duplicate(アインACFUNC)		アイン#GEFF03	747
pay_dun01,231,77,4		duplicate(アインACFUNC)		アイン#PAYD01	747
gef_fild11,369,297,4	duplicate(アインACFUNC)		アイン#GEFF11	747
gef_fild12,125,187,4	duplicate(アインACFUNC)		アイン#GEFF12	747
mjolnir_05,170,285,4	duplicate(アインACFUNC)		アイン#MJOL05	747
gef_dun01,247,234,4		duplicate(アインACFUNC)		アイン#GEFD01	747
