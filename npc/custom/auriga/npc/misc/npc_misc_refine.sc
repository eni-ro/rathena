...
function	script	BlackSmith	{
	...
	next;
	set .@i,select(
			getequipisequiped2(1)? "^nItemID^"+getequipid2(1): "",
			getequipisequiped2(2)? "^nItemID^"+getequipid2(2): "",
			getequipisequiped2(3)? "^nItemID^"+getequipid2(3): "",
			getequipisequiped2(4)? "^nItemID^"+getequipid2(4): "",
			getequipisequiped2(5)? "^nItemID^"+getequipid2(5): "",
			getequipisequiped2(6)? "^nItemID^"+getequipid2(6): "",
			"やっぱりやめる");
	if(.@i == 7) {
		mes "[鍛冶職人]";
		mes "また来るんだな。";
	...
	if(getequiprefinerycnt2(.@i)<.max) {
		...
		case 1:
			...
			break;
		case 2:
			set .@num,.max - getequiprefinerycnt2(.@i);
			mes "["+getarg(0)+"]";
			mes "君が選んだ装備を精錬するには";
