...
moc_fild12,261,318,3	script	怪しい男	998,{
...
einbroch,54,97,3	script	ガード	10062,{
	if(CHANGE_RB == 1) {
		mes "‐手配書に書かれていた";
...
job_gun,56,26,0	warp	リベリオン室内#1	2,2,einbroch,49,97
job_gun,18,10,0	warp	リベリオン室内#2	2,2,job_gun,81,29
job_gun,165,22,0	warp	リベリオン室内#3	2,2,job_gun,197,44
job_gun,197,49,0	warp	リベリオン室内#4	2,2,job_gun,157,21

job_gun,68,33,3	script	冷静なリベリオン	10063,{
	if(Job == Job_Rebellion) {
		if(checkquest2(121920)&0x8) {
			mes "[冷静なリベリオン]";
			mes "やあ、同志よ。";
			mes "銃の扱いには常に気を配るんだぞ。";
			mes "その銃が我々の象徴だからな。";
...
job_gun,78,33,6	script	怪しいガンマン	731,{
	if(Job == Job_Rebellion) {
		if(checkquest2(121915)&0x8) {
			mes "[怪しいガンマン]";
			mes "よお、同志よ。";
			mes "自由を守るために";
			mes "銃の道を究めているか？";
...
job_gun,74,24,6	script	酔っ払いの男	887,{
	if(Job == Job_Rebellion) {
		if(checkquest2(121905)&0x8) {
			mes "[酔っ払いの男]";
			mes "ぐおおおおおおお……";
			mes "　";
			mes "‐男は大きないびきをかいて";
...
job_gun,64,33,6	script	不安な男	748,{
	if(Job == Job_Rebellion) {
		if(checkquest2(121920)&0x8) {
			mes "[不安な男]";
			mes "彼らの象徴は自由らしいですけど、";
...
job_gun,86,29,3	script	クルベル	10062,{
	if(Job == Job_Rebellion) {
		if(checkquest2(121910)&0x8) {
			if(checkquest2(121925)&0x8 == 0) {
				setquest 121925;
				compquest 121925;
			}
			mes "[クルベル]";
...
job_gun,19,24,3	script	ルウィーニ#1	10063,{
	if(CHANGE_RB == 2) {
		mes "[ルウィーニ]";
		mes "やあ。君が";
		mes "クルベルの言っていた";
...
job_gun,117,46,3	script	ルウィーニ#2	10063,{
...
job_gun,165,18,1	script	溶鉱炉制御機#1	844,{
	if(checkquest2(12342)) {
		mes "‐溶鉱炉から完成した製品を";
		mes "　抽出しますか？‐";
		next;
...
job_gun,165,18,1	script	溶鉱炉制御機#2	844,{
	if(checkquest2(12342)) {
		mes "‐完成した鉄製加工物の";
		mes "　抽出が行われています。";
		mes "　完了するまで";
...
OnStart:
	...
	for(set .@i,0; .@i<30; set .@i,.@i+1) {
		...
		set .@x,rand(127,143);
		set .@y,rand(26,29);
		dropitem "job_gun",.@x,.@y,6746,1,0;
		if(rand(100) < 50) {
			set .@x,rand(127,143);
			set .@y,rand(26,29);
			dropitem "job_gun",.@x,.@y,6747,1,0;
		}
		sleep 500;
	}
...
job_gun,130,31,1	script	加熱炉#3	844,{
	end;
OnStart:
	misceffect 96;
	sleep 500;
...
job_gun,136,31,1	script	加熱炉#4	844,{
	end;
OnStart:
	misceffect 96;
	sleep 500;
...
job_gun,141,31,1	script	加熱炉#5	844,{
	end;
OnStart:
	misceffect 96;
	sleep 500;
...
job_gun,214,36,1	script	全自動金敷#1	844,{
	if(checkquest2(12342)) {
		if(countitem(13118)) {
			mes "‐非正規ルートで精製された";
			mes "　銃器の痕跡を発見。";
...
job_gun,95,148,10,10	monster	非射撃標的	3170,3,180000,0,0
job_gun,95,148,10,10	monster	射撃標的	3169,5,180000,0,0
job_gun,95,110,10,15	monster	非射撃標的	3170,3,180000,0,0
job_gun,95,110,10,15	monster	射撃標的	3169,17,180000,0,0
job_gun,140,100,15,10	monster	非射撃標的	3170,13,180000,0,0
job_gun,140,100,15,10	monster	射撃標的	3169,7,180000,0,0
job_gun,140,150,15,10	monster	非射撃標的	3170,4,180000,0,0
job_gun,140,150,15,10	monster	射撃標的	3169,4,180000,0,0
job_gun,115,160,10,10	monster	非射撃標的	3170,8,180000,0,0
job_gun,115,160,10,10	monster	射撃標的	3169,10,180000,0,0
...
job_gun,88,144,0	warp	射撃場#1	2,2,job_gun,93,123
job_gun,105,97,0	warp	射撃場#2	2,2,job_gun,125,98
job_gun,156,115,0	warp	射撃場#3	2,2,job_gun,157,137
job_gun,152,154,0	warp	射撃場#4	2,2,job_gun,135,154
job_gun,130,146,0	warp	射撃場#5	2,2,job_gun,120,130

job_gun,220,132,3	script	シドレン#2	10062,{
	if(Job == Job_Rebellion) {
		mes "[シドレン]";
		mes "やあ。調子はどうかな？";
		mes "自由を体現するために";
...
job_gun,220,138,4	shop	小道具自販機	564,13200:1

job_gun,216,138,4	script	カルレックス	47,{
	mes "[カルレックス]";
	mes "転職前に不要な物を預かります。";
	next;
