...
yuno_in02,88,164,5	script	メテウスシルプ	742,{
	if(TRANSMIGRATE == 0) {
...
//==============================================================
yuno_in02,93,207,0	script	ユミルの書	111,{
	if(Upper == UPPER_HIGH) {
		mes "[ユミルの書]";
		mes "……栄光の神殿への入り口は";
	...
	mes "それゆえ私はここに記録を残そう。";
	next;
	if(!checkquest2(1000)) setquest 1000;
	mes "[ユミルの書]";
...
//==============================================================
yuno_in05,49,43,0	script	ユミルの心臓	111,{
...
valkyrie,48,86,4	script	ヴァルキリー	811,{
...
//==============================================================
valkyrie,44,33,5	script	テレポーター	124,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[テレポーター]";
			mes "転生おめでとう！";
	...
	switch (select("プロンテラ","モロク","フェイヨン","ゲフェン","アルベルタ","イズルード","アルデバラン","コモド","ジュノー")) {
		case 1: savepoint "prontera",116,72; 	break;
		case 2: savepoint "morocc",156,46; 		break;
		case 3: savepoint "payon",160,58; 		break;
		case 4: savepoint "geffen",120,39; 		break;
		case 5: savepoint "alberta",117,56; 	break;
		case 6: savepoint "izlude",91,105; 		break;
		case 7: savepoint "aldebaran",160,109; 	break;
		case 8: savepoint "comodo",209,143; 	break;
		case 9: savepoint "yuno",328,101; 		break;
	}
	warp "SavePoint",0,0;
	end;
}
...
//==============================================================
valkyrie,44,39,5	script	ロードナイト	56,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[ロードナイト]";
			mes "転生おめでとう！";
...
//==============================================================
valkyrie,44,42,5	script	ハイプリースト	60,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[ハイプリースト]";
			mes "転生のお祝いを申し上げます。";
...
//==============================================================
valkyrie,44,47,5	script	ハイウィザード	735,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[ハイウィザード]";
			mes "転生おめでとうございます。";
	...
	callfunc "JobChanger","ハイウィザード",Job_Mage,Job_Wizard;
	mes "[ハイウィザード]";
	mes "栄光のヴァルハラへようこそ。";
	mes "しばし休息をおとりください。";
	mes "戦士に光あれ！";
...
//==============================================================
valkyrie,44,50,5	script	ホワイトスミス	731,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[ホワイトスミス]";
			mes "転生おめでとう！";
...
//==============================================================
valkyrie,44,55,5	script	スナイパー	727,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[スナイパー]";
			mes "転生お祝い申し上げます。";
...
//==============================================================
valkyrie,44,58,5	script	アサシンクロス	725,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[アサシンクロス]";
			mes "転生お祝い申し上げる。";
...
//==============================================================
valkyrie,53,39,3	script	パラディン	752,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[パラディン]";
			mes "転生の祝いを申し上げよう。";
...
//==============================================================
valkyrie,53,42,3	script	チャンピオン	52,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[チャンピオン]";
			mes "転生おめでとう！";
...
//==============================================================
valkyrie,53,47,3	script	プロフェッサー	743,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[プロフェッサー]";
			mes "転生おめでとう。";
	...
	callfunc "JobChanger","プロフェッサー",Job_Mage,Job_Sage;
	mes "[プロフェッサー]";
...
//==============================================================
valkyrie,53,58,3	script	チェイサー	747,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[チェイサー]";
			mes "転生おめでと～！";
...
//==============================================================
valkyrie,53,50,3	script	クリエイター	122,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[クリエイター]";
			mes "転生おめでとう。";
...
//==============================================================
valkyrie,53,54,3	script	クラウン	741,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[クラウン]";
			mes "転生お祝い申し上げます。";
...
//==============================================================
valkyrie,53,56,3	script	ジプシー	101,{
	if(Upper != UPPER_HIGH) {
		if(rand(3)) {
			mes "[ジプシー]";
			mes "転生おめでとう！";
