...
ven_in01,93,96,5	script	スターター装備交換	4_M_GUILLOTINE,{ //4_SYS_MSG
	...
	next;
	setarray .@list_iid,
		1150,	19468,	15418,	20970,	22227,	32261,	//天秤宮(RK)
		640002,	5599,	450015,	480009,	470014,	490010,	//処女宮(AB)
		26167,	19496,	15426,	20972,	22231,	32266,	//宝瓶宮(WL)
		28047,	19471,	15424,	20976,	22233,	32267,	//天蝎宮(GX)
		700010,	5739,	450140,	480050,	470045,	490041,	//人馬宮(連)
		1335,	19449,	15412,	20954,	22216,	32253,	//巨蟹宮(メカ)
		640006,	5514,	450133,	480041,	470038,	490034,	//双魚宮(皿)
		28643,	15823,	15447,	20983,	2400,	32293,	//ポルックス(星帝)
		28784,	15824,	15448,	20984,	22242,	32294;//プロキオン(ソウルリーパー)
	.@menu$ = "";
	for(.@i = 0; .@i < 6; .@i++)
	...
	for(.@i = 1; .@i <= .@req_num; .@i++){
		mes sprintf("交換に出す%d個目の装備を選んでくれ。",.@i);
		mes "";
