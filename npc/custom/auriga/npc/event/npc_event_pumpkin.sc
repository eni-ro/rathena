//カボチャ部屋の設定
//(本来はforce_1-1 しかし、アリーナクエストと重複するため、同MAP異名の2-1を使用)
//	→ 現在2-1も重複するのでこのスクリプトを利用したい場合は適切な場所に移動すること

force_2-1,100,100,0,0	monster	ザック	1130,200,50000,25000
force_2-1,100,100,0,0	monster	ブラッディナイト	1268,5,50000,25000
force_2-1	mapflag	nosave	prontera,156,191
force_2-1	mapflag	nomemo	dummy
force_2-1	mapflag	noteleport	dummy
force_2-1	mapflag	pvp	dummy
//イベントホールの設定
pvp_2vs2	mapflag	nosave	prontera,156,191
pvp_2vs2	mapflag	nomemo	dummy
pvp_2vs2	mapflag	noteleport	dummy
pvp_2vs2	mapflag	pvp	dummy


prt_monk,225,180,0	script	助教	89,{
	mes "[玄武]";
	mes "作物が育つ様を見ていると";
	mes "神の寵愛を感じずにはいられない。";
	next;
...
gef_fild00,77,212,4	script	生意気な子	716,{
	mes "[生意気な子]";
	mes "ちっ、あのピエロめ・・・";
	mes "僕のキャンディーを奪った上に、";
	mes "もっといい物持って来いだなんて！";
...
pvp_2vs2,40,30,4	script	臆病者	67,{
	mes "[臆病者]";
	mes "おれを殴る気なのか？！";
	mes "く、くるな～～～！！";
	mes "おまえなんかどっかに飛んでいけ！";
	close2;
	warp "prt_monk",95,212;
	end;
}

// ■ PUMP_EVE -> 0～2

xmas_in,27,103,4	script	太っちょおばさん#pump	701,{
	if(PUMP_EVE != 1) {
		mes "[タシンチェ]";
		mes "メリークリスマスー";
		mes "ほっほほほ～";
...
//--------------------------------------------------
prt_monk,218,175,4	script	修道士	49,{
	if(PUMP_EVE == 0) {
		mes "[修道士]";
		mes "今年は何年かぶりに";
		mes "魔法のカボチャが取れました。";
...
prt_monk,227,162,4	script	修行僧	753,{
	if(PUMP_EVE != 1) {
		mes "[修行僧]";
		mes "おう、あんたもう";
		mes "魔法のかぼちゃは貰ったかい？";
	...
	mes "近くまで連れて行ってやろう。";
	close2;
	warp "xmas_fild01",150,150;
	end;
}

gef_fild00,73,209,4	script	聖なるピエロ	715,{
	if(PUMP_EVE != 2) {
	...
	if(select("Trick","Treat")==1) {	//Trick選択
		...
		case 2:		//GH最下層地下洞窟
			mes "[聖なるピエロ]";
			mes "これより探検隊を派遣します！";
			warp "gl_dun02",138,72;
			break;
		case 3:		//タートルアイランド久陽宮
			mes "[聖なるピエロ]";
			mes "君もこれで王様どぁぁぁぁ！！！";
			warp "tur_dun04",100,111;
			break;
		case 4:		//イズルード海底神殿
			mes "[聖なるピエロ]";
			mes "本日開店！魚観覧所！！";
			warp "iz_dun04",130,227;
			break;
		case 5:		//時計塔地上
			mes "[聖なるピエロ]";
			mes "今日から君もここの店長だ！";
			warp "c_tower4",119,179;
			break;
		case 6:		//沈没船2F
			mes "[聖なるピエロ]";
			mes "船長ぉぉぉぉ！！！";
			warp "tresure02",100,164;
			break;
		case 7:		//カボチャ部屋
			mes "[聖なるピエロ]";
			mes "お一人さまご案内～";
