...
// エンチャントNPC
//------------------------------------------------------------
glast_01,212,273,4	script	魔法職人	66,{
	mes "[魔法職人]";
	mes "時空ブーツのエンチャントですか？";
	mes "スロットを追加したブーツは";
	mes "エンチャントの対象ではないので";
...
// 交換NPC
//------------------------------------------------------------
glast_01,210,273,5	script	フギンの執事	69,{
	mes "[フギンの執事]";
	mes "時間の結晶1個と時空ブーツを";
	mes "交換しております。";
	mes "上位の時空ブーツとの交換は";
...
// スロットエンチャントNPC
//------------------------------------------------------------
glast_01,210,270,8	script	細工職人	726,{
	mes "[細工職人]";
	mes "そこの魔法職人は";
	mes "ただ強化すればそれで良いと";
	mes "考えているようだが私は違う。";
...
// 転送NPC
//------------------------------------------------------------
glast_01,195,273,5	script	フギンの従者#1	123,{
	mes "[フギンの従者]";
	mes "フギン師匠のように、";
	mes "次元と時間の狭間を";
	mes "自由に移動させることは";
...
gl_chyard_,148,149,3	script	フギンの従者#2	123,{
	mes "[フギンの従者]";
	mes "帰りますか？";
	next;
	if(select("帰る","帰らない") == 2) {
	...
	mes "では、お送りします。";
	close2;
	warp "glast_01",195,266;
	end;
}

gl_cas02_,104,36,3	script	フギンの従者#3	123,{
	mes "[フギンの従者]";
	mes "帰りますか？";
	next;
	if(select("帰る","帰らない") == 2) {
...
gl_cas02_,105,193,0	script	nightmare_gl_cas02	45,1,1,{
	mes "‐外に出ますか？‐";
	next;
	if(select("出る","出ない") == 2) {
		close;
