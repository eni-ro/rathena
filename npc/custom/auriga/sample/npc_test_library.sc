...
function	script	StrTrim	{
	set .@dup$,getarg(0);
	while(1) {	// 前方検索
		set .@buf$,substr2(.@dup$,.@i,1);
		if(.@buf$ == "" || .@buf$ != " ")
			break;
		set .@i,.@i+1;
	}
	return substr2(.@dup$,.@i);
}
...
function	script	StrRTrim	{
	...
	return (.@i == -1)? substr2(.@dup$,0): substr2(.@dup$,0,.@i+1);
}
...
function	script	StrCSpn	{
	...
	for(set .@i,0; .@i < .@max; set .@i,.@i+1) {		// <str1>の走査
		for(set .@j,0; .@j < .@len; set .@j,.@j+1) {	// <str2>の走査
			if( substr2(.@src$,.@i,1) == substr2(.@dst$,.@j,1) )
				return .@i;
		}
	}
	return -1;
...
function	script	StrWCSpn	{
	...
	for(set .@i,0; .@i < .@max; set .@i,.@i+2) {		// <str1>の走査
		for(set .@j,0; .@j < .@len; set .@j,.@j+2) {	// <str2>の走査
			if( substr2(.@src$,.@i,2) == substr2(.@dst$,.@j,2) )
				return .@i;
		}
	}
	return -1;
...
function	script	StrDelete	{
	...
	set .@buf$,getarg(0);
	return substr2(.@buf$,0,.@n) + substr2(.@buf$,.@n+1);
}
...
function	script	StrShift	{
	...
	return substr2(.@buf$,0,.@n - .@len) + substr2(.@buf$,.@n);
}
...
function	script	StrInsert	{
	...
	set .@buf$,getarg(0);
	return substr2(.@buf$,0,.@n) + getarg(2) + substr2(.@buf$,.@n);
}
...
function	script	StrReplace	{
	...
	return substr2(.@buf$,0,.@n) + getarg(2) + substr2(.@buf$,.@n + getstrlen(getarg(1)));
}
...
function	script	StrAReplace	{
	...
	for(set .@i,0; .@i <= .@max; set .@i,.@i+1) {
		if( substr2(.@src$,.@i,.@len) == .@dst$ ) {
			set .@ret$,.@ret$ + substr2(.@src$,.@pos,.@i - .@pos) + getarg(2);
			set .@pos,.@i + .@len;	// 次の検索開始位置を保存
			set .@i,.@pos-1;	// 次の検索位置まで飛ばす
		}
	}
	return .@ret$ + substr2(.@src$,.@pos);	// 残りを末尾まで繋いで終了
}
...
function	script	StrWReverse	{
	...
	for(set .@i,.@len-2; .@i >= 0; set .@i,.@i-2) {
		set .@ret$,.@ret$ + substr2(.@buf$,.@i,2);
	}
	if(.@i != 0)	// 文字列長が奇数のときは余った先頭1バイトを足す
		set .@ret$,.@ret$ + substr2(.@buf$,0,1);
	return .@ret$;
}
...
function	script	StrShuffle	{
	...
	for(set .@i,.@len-1; .@i >= 0; set .@i,.@i-1) {
		if(.@i == 0) {
			...
			set .@rand,rand(.@i);
			set .@ret$,.@ret$ + substr2(.@dup$,.@rand,1);
...
function	script	StrArray	{
	...
	for(set .@i,0; .@i < .@max && .@i < 128; set .@i,.@i+1) {
		set getelementofarray(getarg(2),.@i),substr2(.@buf$,.@n+.@i,1);
	}
	return .@i;	// 格納した個数を返す
}
...
function	script	StrSplit	{
	...
	for(set .@i,0; .@i <= .@max; set .@i,.@i+1) {
		if( substr2(.@src$,.@i,.@len) == .@dst$ ) {
			set getelementofarray(getarg(2),.@idx),substr2(.@src$,.@pos,.@i - .@pos);
			set .@idx,.@idx+1;
			set .@pos,.@i + .@len;	// 次の検索開始位置を保存
			set .@i,.@pos-1;	// 次の検索位置まで飛ばす
		}
	}
	set getelementofarray(getarg(2),.@idx),substr2(.@src$,.@pos);	// 残りを末尾まで繋ぐ
...
// substr2関数の仕様
//-----------------------------------------------
prontera,157,191,4	script	文字列抽出テスト	112,{
	set .@buf$,"ABCDEFGH";
	mes "[offsetの確認]";
	mes "正のoffset →";
	mes "　  0   1   2   3   4   5   6   7";
	...
	mes "-3: A   B   C   D   E   -   -   -";
	next;
	mes "01 : " + substr2(.@buf$,2);		// CDEFGH
	mes "02 : " + substr2(.@buf$,1,4);	// BCDE
	mes "03 : " + substr2(.@buf$,3,8);	// DEFGH
	mes "04 : " + substr2(.@buf$,-3);	// FGH
	mes "05 : " + substr2(.@buf$,-5,4);	// DEFG
	mes "06 : " + substr2(.@buf$,1,-3);	// BCDE
	mes "07 : " + substr2(.@buf$,-5,-2);	// DEF
	mes "08 : " + substr2(.@buf$,-1,1);	// H
	mes "09 : " + substr2(.@buf$,-4,5);	// EFGH
	mes "10 : " + substr2(.@buf$,0,0);	// [nothing]
	mes "11 : " + substr2(.@buf$,4,0);	// [nothing]
	mes "12 : " + substr2(.@buf$,-9);
	mes "13 : " + substr2(.@buf$,10);
	mes "14 : " + substr2(.@buf$,1,-12);
	mes "15 : " + substr2(.@buf$,6,-3);
	mes "16 : " + substr2(.@buf$,-3,-4);
	mes "17 : " + substr2(.@buf$,6,-2);
	next;
	mes "[テスト]";
	mes "文字列を入力";
	next;
...
// 文字列処理使用例
//-----------------------------------------------
prontera,159,191,4	script	文字列処理テスト	113,{
