...
// IL_MOON_QUE -> 0～10
//==============================================================================
payon,164,229,5	script	元老サンユル#ilp01	78,{
	if(BaseLevel < 170) {
		mes "[サンユル]";
		mes "何か怪しい事が";
		mes "起きている予感がします。";
...
payon,167,229,3	script	元老ジャギョム#ilp02	901,{
	if(IL_MOON_QUE == 0) {
		mes "[ジャギョム]";
...
pay_arche,46,135,5	script	ムヒョン#ilp04	644,{
	end;
OnInit:
...
pay_arche,44,133,7	script	兵士シンジュウヒ#ilp05	708,{
	end;
OnInit:
	cloakonnpc strnpcinfo(0);
	end;
...
payon,103,322,4	script	フェイヨン兵士 #ilp06	708,{
	if(IL_MOON_QUE == 2) {
		mes "[ジュウヒ]";
		mes "前に洞窟の前で見た";
		mes "冒険者じゃないか。";
...
payon_in03,33,95,3	script	ムヒョン#ilp07	644,{
	if(IL_MOON_QUE < 2) {
		mes "[ムヒョン]";
		mes "イェジョル先生を探しに来たなら";
		mes "もっと奥の部屋に行って下さい。";
...
payon_in03,33,93,1	script	青年学士#ilp08	92,{
	if(IL_MOON_QUE < 2) {
		mes "[青年学士]";
		mes "フェイヨンは昔から";
		mes "神獣たちと一緒に暮らす街でした。";
...
payon_in03,30,96,5	script	ジャギョム#ilp09	901,{
	if(IL_MOON_QUE < 2) {
		mes "[ジャギョム]";
		mes "宮内では静かにしなさい。";
		close;
...
payon_in03,31,96,5	script	#狐魂ilp10	1180,{
	end;
OnInit:
	cloakonnpc strnpcinfo(0);
	end;
...
payon_in03,35,96,3	script	#狐魂ilp11	1180,{
	end;
OnInit:
	cloakonnpc strnpcinfo(0);
	end;
...
payon_in03,31,93,7	script	#狐魂ilp12	1180,{
	end;
OnInit:
	cloakonnpc strnpcinfo(0);
	end;
...
payon_in03,35,93,1	script	#狐魂ilp13	1180,{
	end;
OnInit:
	cloakonnpc strnpcinfo(0);
	end;
...
pay_dun03,140,46,3	script	揺らぐ空間#ilp14	10043,{
	if(IL_MOON_QUE < 5) {
		mes "怪しい気が揺らいでいる。";
		mes "不気味な感じがする。";
		close;
...
pay_dun03,138,43,7	script	青年学士#ilp15	92,{
	end;
OnInit:
	cloakonnpc strnpcinfo(0);
	end;
...
pay_dun03,142,45,3	script	兵士シンジュウヒ#ilp16	708,{
	end;
OnInit:
	cloakonnpc strnpcinfo(0);
	end;
...
pay_d03_i,149,45,4	script	学士ホドニ#ilp18	92,{
	if(IL_MOON_QUE == 6) {
		mes "[ホドニ]";
		mes "ちょっとこっちを見て下さい！";
		mes "先ほど私たちがいた";
...
pay_d03_i,152,45,3	script	兵士シンジュウヒ#ilp17	708,{
	if(IL_MOON_QUE <= 6) {
		mes "[ジュウヒ]";
		mes "何の対策もないまま";
		mes "入った時から感じてましたが、";
...
pay_d03_i,140,46,3	script	揺らぐ空間#ilp10	10043,{
	mes "元の廃村に戻ることができる。";
	mes "どうしようかな？";
	next;
	if(select("戻る","戻らない") == 2) {
...
pay_d03_i,82,237,3	script	村人#ilp17	75,{
	mes "[村人]";
	mes "あ……ああ……";
	close;
}

pay_d03_i,254,206,3	script	村人#ilp18	90,{
	mes "[村人]";
	mes "それは違う……！";
	mes "魔法を、魔法を止めて……！";
	mes "く、崩れる!!きゃああ!!";
...
pay_d03_i,81,136,3	script	村人#ilp19	59,{
	mes "[村人]";
	mes "いったい私たちが";
	mes "何をしたと？";
	mes "神獣と魔物も";
...
pay_d03_i,160,260,3	script	村人#ilp21	59,{
	mes "[村人]";
	mes "いったい、あなた達は……。";
	mes "あの怪しい魔法使いは";
	mes "なぜ私たちを……。";
...
pay_d03_i,43,150,5	script	村人#ilp22	75,{
	mes "[村人]";
	mes "どうか私たちを";
	mes "放っておいてください！";
	next;
...
pay_d03_i,0,0,0	script	#IllMoonlight	-1,{
OnInit:
	for(set .@i,0;.@i<5;set .@i,.@i+1)
		areamonster "pay_d03_i",40,35,160,115,"--ja--",3759+rand(3),1,strnpcinfo(0)+"::OnSummon1";
	for(set .@i,0;.@i<5;set .@i,.@i+1)
		areamonster "pay_d03_i",40,140,160,260,"--ja--",3759+rand(3),1,strnpcinfo(0)+"::OnSummon2";
	for(set .@i,0;.@i<5;set .@i,.@i+1)
		areamonster "pay_d03_i",150,180,270,260,"--ja--",3759+rand(3),1,strnpcinfo(0)+"::OnSummon3";
	end;

OnSummon1:
	if(.mob1 == 0 && rand(1000) == 0) {
		set .mob1,callmonster("pay_d03_i",96,83,"真理の魔法使い",3764,strnpcinfo(0)+"::OnKilled1");
...
OnSummon2:
	if(.mob2 == 0 && rand(1000) == 0) {
		set .mob2,callmonster("pay_d03_i",94,188,"真理の魔法使い",3764,strnpcinfo(0)+"::OnKilled2");
...
OnSummon3:
	if(.mob3 == 0 && rand(1000) == 0) {
		set .mob3,callmonster("pay_d03_i",210,221,"真理の魔法使い",3764,strnpcinfo(0)+"::OnKilled3");
