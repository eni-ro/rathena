...
// ブワヤの巣
//------------------------------------------------------------
1@ma_c,0,0,0	script	BuwayaControl	-1,{
OnStart:
	if(.flag > 0)
		end;
	set .flag,1;
...
1@ma_c,33,112,0	script	#buwaya_damage	139,7,7,{
	end;
OnStart:
	initnpctimer;
	end;
...
1@ma_c,29,110,5	script	拉致された人たち#1	575,{
	mes "[拉致された人たち]";
...
1@ma_c,36,110,5	script	拉致された人たち#2	574,{
	mes "[拉致された人たち]";
	mes "誰か非力なワシの代わりに";
	mes "ブワヤの弱点を攻撃してくれんか……。";
	next;
...
1@ma_c,3,3,0	script	#buwaya_boxmob	139,{
OnStart:
	if(getmapmobs(getmdmapname("1@ma_c"),getmdnpcname("#buwaya_boxmob")+"::OnKilled") < 1) {
...
OnClose:
	killmonster getmdmapname("1@ma_c"),getmdnpcname("#buwaya_boxmob")+"::OnKilled";
	end;
OnKilled:
	if(getmapmobs(getmdmapname("1@ma_c"),getmdnpcname("#buwaya_boxmob")+"::OnKilled") < 1) {
...
1@ma_c,34,118,0	script	#buwaya_boxout	45,7,7,{
OnTouch:
	set .@x,rand(1,20) + 97;
	set .@y,rand(1,20) + 74;
	warp getmdmapname("1@ma_c"),.@x,.@y;
	end;
}

1@ma_c,97,74,0	script	#buwaya_trap1	139,5,5,{
OnTouch:
	misceffect 191,"";
	warp getmdmapname("1@ma_c"),33,112;
	end;
}
1@ma_c,97,94,0	duplicate(#buwaya_trap1)	#buwaya_trap2	139,5,5
1@ma_c,117,94,0	duplicate(#buwaya_trap1)	#buwaya_trap3	139,5,5
1@ma_c,117,74,0	duplicate(#buwaya_trap1)	#buwaya_trap4	139,5,5
1@ma_c,117,54,0	duplicate(#buwaya_trap1)	#buwaya_trap5	139,5,5
1@ma_c,97,54,0	duplicate(#buwaya_trap1)	#buwaya_trap6	139,5,5
1@ma_c,77,54,0	duplicate(#buwaya_trap1)	#buwaya_trap7	139,5,5
1@ma_c,77,74,0	duplicate(#buwaya_trap1)	#buwaya_trap8	139,5,5
1@ma_c,77,94,0	duplicate(#buwaya_trap1)	#buwaya_trap9	139,5,5

1@ma_c,1,1,0	script	#buwaya_bunshin	139,{
	end;
OnStart:
	initnpctimer;
	end;
...
OnTimer105000:
	killmonster getmdmapname("1@ma_c"),getmdnpcname("#buwaya_bunshin")+"::OnKilled";
	initnpctimer;
	end;
OnKilled:
	if(getmapmobs(getmdmapname("1@ma_c"),getmdnpcname("#buwaya_bunshin")+"::OnKilled") < 1) {
		initnpctimer;
	}
	end;
OnClose:
...
1@ma_c,2,2,0	script	#buwaya_boss	139,{
	end;
OnStart:
	areamonster getmdmapname("1@ma_c"),90,67,104,81,"ブワヤ",2319,1,getmdnpcname("#buwaya_boss")+"::OnKilled";
	end;
OnKilled:
	set .@map$, getmdmapname("1@ma_c");
...
1@ma_c,3,3,0	script	#buwaya_exit_mob	139,{
	end;
OnStart:
	initnpctimer;
	end;
...
OnClose:
	stopnpctimer;
	killmonster getmdmapname("1@ma_c"),getmdnpcname("#buwaya_exit_mob")+"::OnKilled";
	end;
}

1@ma_c,28,57,0	script	#buwaya_caveout	45,2,2,{
OnTouch:
	mes "‐外に出ますか？‐";
	next;
	if(select("出る","出ない") == 2) {
